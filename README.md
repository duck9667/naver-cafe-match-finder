...
- 한 번 출력된 글의 링크는 `seen_links.json`에 저장되어, 이후 실행에서는 기본적으로 "새 글"만 출력한다 (최초 1회는 검색되는 모든 글이 새 글로 잡힌다는 점 주의).
- 더 이상 추적하지 않고 다시 전체를 새 글로 보고 싶으면 `seen_links.json`을 삭제하면 된다.
- 매번 모든 매칭글을 다시 보고 싶다면(중복 알림을 감수하고 놓치는 걸 방지하는 용도) `.env`에 `DEDUPE_SEEN=false`를 추가하면 된다 — 이 경우 같은 글이 실행마다 반복해서 출력/발송된다.
